...
// ---------- VernaMall 3D coin: real front/back faces, embossed metallic "VR", sparkle ----------
const VernaCoin = React.forwardRef(function VernaCoin({ t, size = 100 }, headerRef) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const rotY = (y * 0.4) % 360;
        if (stageRef.current) {
          stageRef.current.style.transform = `rotateY(${rotY}deg)`;
        }
      });
    };
  ...
  const bg0 = t.mode === 'dark' ? '#0A0F1C' : '#FFFCF5';
  const soft = t.goldSoft;
  const mid = t.gold;
  const deep = t.mode === 'dark' ? t.purple : '#6E4A0E';
  const rim = `0 0 0 3px ${bg0}, 0 0 0 5px ${mid}, 0 0 0 7px ${deep}, 0 ${Math.round(size * 0.12)}px ${Math.round(size * 0.3)}px rgba(0,0,0,0.55)`;
  ...
  const sparkPositions = [
    ...
    { top: '14%', left: '82%', delay: 0.4 },
    { top: '78%', left: '10%', delay: 0.9 },
    { top: '88%', left: '70%', delay: 1.3 },
    { top: '48%', left: '-4%', delay: 0.6 },
  ];

  return (
    <div ref={headerRef} style={{ position: 'relative', width: size, height: size }}>
      <div
        style={{
          position: 'absolute',
          inset: -size * 0.35,
          background: `radial-gradient(circle, ${mid}4D 0%, transparent 70%)`,
          filter: 'blur(6px)',
          zIndex: 0,
        }}
      />
      {sparkPositions.map((s, i) => (
        <span
          ...
          style={{
            ...
            borderRadius: '50%',
            background: '#FFFBEF',
            boxShadow: '0 0 6px 2px rgba(255,246,214,0.9)',
            animationDelay: `${s.delay}s`,
            zIndex: 2,
          }}
        />
      ))}
      <div style={{ perspective: 900, width: size, height: size, position: 'relative', zIndex: 1 }}>
        <div
          ref={stageRef}
          style={{
            width: size,
        ...
        >
          <div
            style={{
              ...faceBase,
              background: `radial-gradient(circle at 30% 24%, ${soft} 0%, ${mid} 55%, ${deep} 100%)`,
              boxShadow: rim,
            }}
          >
            <span
              style={{
                ...
                backgroundImage: `linear-gradient(180deg, #FFF7DD 0%, ${mid} 45%, ${deep} 100%)`,
                WebkitBackgroundClip: 'text',
                backgroundClip: 'text',
                color: 'transparent',
                textShadow: '0 1px 0 rgba(255,255,255,0.55), 0 2px 4px rgba(0,0,0,0.55)',
              }}
            >
              VR
            </span>
...
export default function RaffleApp() {
  ...
  // ---------- welcome popup: shows 3s, then flies into the header logo ----------
  const [welcomeStage, setWelcomeStage] = useState('visible'); // 'visible' | 'flying' | 'hidden'
  const [welcomeFlyStyle, setWelcomeFlyStyle] = useState({});
  const welcomeCardRef = useRef(null);
  const headerCoinRef = useRef(null);

  useEffect(() => {
    const showTimer = setTimeout(() => {
      const target = headerCoinRef.current?.getBoundingClientRect();
      const source = welcomeCardRef.current?.getBoundingClientRect();
      if (target && source) {
        const dx = (target.left + target.width / 2) - (source.left + source.width / 2);
        const dy = (target.top + target.height / 2) - (source.top + source.height / 2);
        const scale = Math.max(0.14, target.width / source.width);
        setWelcomeFlyStyle({ transform: `translate(${dx}px, ${dy}px) scale(${scale})`, opacity: 0 });
      } else {
        setWelcomeFlyStyle({ transform: 'translateY(-70px) scale(0.3)', opacity: 0 });
      }
      setWelcomeStage('flying');
    ...
    <div dir="rtl" style={styles.page}>
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'Vazirmatn', sans-serif; }
        ...
        .verna-spark { animation: sparkTwinkle 2.2s ease-in-out infinite; }
        @keyframes sparkTwinkle { 0%, 100% { opacity: 0; transform: scale(0.3); } 50% { opacity: 1; transform: scale(1); } }
        .welcome-pop-in { animation: welcomePopIn 0.5s cubic-bezier(.2,.9,.25,1.3) both; }
        @keyframes welcomePopIn { from { opacity: 0; transform: scale(0.55); } to { opacity: 1; transform: scale(1); } }
        .welcome-backdrop-in { animation: welcomeBackdropIn 0.35s ease both; }
        @keyframes welcomeBackdropIn { from { opacity: 0; } to { opacity: 1; } }
      ...
      {welcomeStage !== 'hidden' && (
        <div
          ...
          style={{
            ...
            justifyContent: 'center',
            padding: 20,
            opacity: welcomeStage === 'flying' ? 0 : 1,
            transition: welcomeStage === 'flying' ? 'opacity 0.5s ease 0.15s' : 'none',
            pointerEvents: welcomeStage === 'flying' ? 'none' : 'auto',
          }}
        >
          <div
            ref={welcomeCardRef}
            className="welcome-pop-in"
            style={{
              ...
              transition: welcomeStage === 'flying' ? 'transform 0.85s cubic-bezier(.5,-0.3,.4,1.25), opacity 0.6s ease' : 'none',
              ...welcomeFlyStyle,
            }}
          >
            <VernaCoin t={t} size={78} />
            <div style={{ fontWeight: 800, fontSize: 15, color: t.text, textAlign: 'center' }}>
              به ورنامال خوش اومدی ✨
            </div>
          </div>
        </div>
      )}
      ...
      <div style={styles.container}>
        ...
        <div className="hero-in" style={styles.hero}>
          <div style={{ display: 'flex', justifyContent: 'center', margin: '0 0 16px' }}>
            <VernaCoin t={t} size={92} ref={headerCoinRef} />
          </div>
