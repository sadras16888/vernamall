...
// ---------- VernaMall 3D emblem (theme-matched, spins/tilts on scroll) ----------
function VernaLogo3D({ t }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const rotY = (y * 0.3) % 360;
        const rotX = 10 + Math.sin(y * 0.012) * 6;
        if (stageRef.current) {
          stageRef.current.style.transform = `rotateX(${rotX}deg) rotateY(${rotY}deg)`;
        }
      });
    };
  ...
  // step-notch shape echoing the bracket in the VernaMall mark
  const notch = 'polygon(0 0, 72% 0, 72% 26%, 100% 26%, 100% 100%, 0% 100%)';
  const plateGrad = t.mode === 'dark'
    ? `linear-gradient(135deg, ${t.goldSoft}, ${t.gold} 45%, ${t.purple} 120%)`
    : `linear-gradient(135deg, ${t.goldSoft}, ${t.gold} 70%)`;
  const stepColor = t.mode === 'dark' ? 'rgba(10,15,28,0.55)' : 'rgba(120,88,20,0.4)';

  return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '0 0 18px' }} dir="ltr">
      <div className="verna-logo-bob" style={{ perspective: 700 }}>
        <div
          ref={stageRef}
          style={{
            display: 'flex',
        ...
        >
          <div
            style={{
              ...
              boxShadow: `0 2px 0 ${stepColor}, 0 4px 0 ${stepColor}, 0 6px 0 ${stepColor}, 0 18px 28px rgba(0,0,0,0.35)`,
              flexShrink: 0,
            }}
          >
            <span
              style={{
                ...
                color: t.mode === 'dark' ? '#0A0F1C' : '#FFFCF5',
                textShadow: t.mode === 'dark' ? '1px 1px 0 rgba(255,255,255,0.15)' : '1px 1px 0 rgba(0,0,0,0.15)',
              }}
            >
              VR
            </span>
          ...
          <span
            style={{
              ...
              letterSpacing: 1,
              fontFamily: "'Vazirmatn', sans-serif",
              backgroundImage: plateGrad,
              WebkitBackgroundClip: 'text',
              backgroundClip: 'text',
              color: 'transparent',
              filter: t.mode === 'dark' ? 'drop-shadow(2px 2px 0 rgba(0,0,0,0.5))' : 'drop-shadow(1px 2px 0 rgba(120,88,20,0.35))',
            }}
          >
            MALL
          </span>
    ...
    <div dir="rtl" style={styles.page}>
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'Vazirmatn', sans-serif; }
        ...
        .verna-logo-bob { animation: vernaBob 3.6s ease-in-out infinite; }
        @keyframes vernaBob { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-6px); } }
